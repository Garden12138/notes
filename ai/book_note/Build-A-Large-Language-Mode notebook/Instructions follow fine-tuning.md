...
* 可以继续尝试：
  ...
  * 调整学习率、batch size、训练轮数和最大序列长度；
  * 增加指令数据的数量、覆盖面和回答质量；
  * 比较 Alpaca、聊天消息等不同提示格式；
  * 只对回答部分计算损失；
  * 使用 LoRA 等参数高效微调方法；
  * 在 SFT 之后进行偏好微调，例如 DPO。

#### 如何在快速变化的前沿领域中保持领先

* 书中的模型、数据集和工具只是学习实现原理的载体。继续学习时应优先抓住稳定概念：自回归目标、提示模板、数据清洗、掩码、评估集隔离和可复现性；具体模型名称和工具接口则以项目当前文档为准。

* 阅读新工作时，先确认其训练数据、基线、评估方法和资源成本，再判断结果是否能迁移到自己的任务。仅看模型规模或单一平均分，通常不足以说明实际效果。
