...
def build_agent_registry() -> tuple[AgentRole, ...]:
    ...
    return (
        AgentRole(
            ...
            expected_inputs=("destination", "preferences"),
            expected_outputs=("attractions",),
            external_capabilities=("amap_poi_search", "unsplash_images"),
        ),
        AgentRole(
            name="weather_query",
            display_name="天气查询 Agent",
            responsibility="查询旅行日期范围内的天气并给出出行提示",
            expected_inputs=("destination", "start_date", "end_date"),
            expected_outputs=("weather_forecast",),
            external_capabilities=("amap_weather",),
        ),
        AgentRole(
            name="hotel_recommendation",
            display_name="酒店推荐 Agent",
            responsibility="结合预算、区域和住宿偏好筛选酒店",
            expected_inputs=("destination", "budget", "accommodation"),
            expected_outputs=("hotels",),
            external_capabilities=("amap_poi_search",),
        ),
        AgentRole(
            name="trip_planning",
            ...
            external_capabilities=("llm_reasoning",),
        ),
    )
