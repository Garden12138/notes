"""Modality-separated perceptual memory."""
...
class PerceptualMemory(PersistentMemory):
    """Keep text descriptions and source metadata for media memories."""

    memory_type = "perceptual"

    def add(self, item: MemoryItem) -> str:
        modality = item.metadata.get("modality", "text")
        if modality not in self.config.perceptual_modalities:
            raise ValueError(f"unsupported perceptual modality: {modality}")
        return super().add(item)

    def _namespace(self, item: MemoryItem) -> str:
        return str(item.metadata.get("modality", "text"))
