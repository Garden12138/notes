...
这是第十四章的持续实践目录。14.1 先实现前后端工程骨架、四层架构契约、SSE 通道和可注入的研究协调器；真实规划、搜索、笔记与报告服务将在后续小节逐步接入。
...
14.1 尚未装配真实服务，因此直接调用全局应用的研究接口会明确返回 `503`，不会生成没有来源的占位报告。后续只需在组合根提供 `DeepResearchAgent` 工厂，无需修改 API 和前端协议。
...
`workflow_demo.py` 使用确定性测试替身验证调用顺序，不访问模型、搜索引擎或文件系统。
