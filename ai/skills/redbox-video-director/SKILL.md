...
description: Use when generating short videos with Bojin official video API. Produces a detailed shot script first, asks the user to confirm it, then chooses between text-to-video, reference-guided, and first-last-frame modes and calls the correct wan2.7 video model with prompt discipline focused on motion, reference elements, and transitions.
...
# Bojin Video Director

Use this skill before calling `video_generate` for Bojin video work. Use `image_generate` for standalone storyboard/keyframe/reference images and pass their returned `absolutePath` values into `video_generate.referenceImages`.
...
- Bojin video generation is locked to the Bojin official video route.
