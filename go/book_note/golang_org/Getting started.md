...
> How to write Go code
  ...
  * 第一个程序
    * 需要编译和运行一个简单的程序，首先选择一个模块路径（如```example/user/hello```）并创建一个声明它的```go.mod```文件。
      ```
      $ mkdir hello
      $ cd hello
      ...
      $ cat go.mod
      ```
    * ```GO```源文件的第一句声明必须是包名。可执行命令必须始终使用包```main```。在该目录下创建名为```hello.go```文件：
      ```
      package main
      ...
      ```
      此命令构建```hello```命令，生成可执行二进制文件。生成的二进制文件安装在```$HOME/go/bin/hello```（```Windows```中为```%USERPROFILE%\go\bin\hello.exe```）。安装目录由```GOPATH```和```GOBIN```环境变量控制。若```GOBIN```已设置，二进制文件将安装在其指定目录。若```GOPATH```已设置，二进制文件将安装在```GOPATH```列表中第一个目录的```bin```子目录。否则，二进制文件将安装在默认```GOPATH```（```$HOME/go```或```%USERPROFILE%\go```）的```bin```子目录。
      使用```go env```命令为将来的```go```命令可移植地设置环境变量的默认值：
      ```
      ...
      $ go env -u GOBIN
      ```
      ```go install```这样的命令使用于包含当前工作目录的模块的上下文中。若工作目录不在```example/user/hello```模块，```go install```可能会失败。为了方便起见，```go```命令接受相对工作目录路径，若未给出其他路径，则默认当前工作目录中的包，故在工作目录中，下面的命令是等价的：
      ```
      $ go install example/user/hello
      $ go install .
      $ go install
      ```
    * 运行程序以确保其正常工作。为了更加方便，将安装目录添加值```PATH```以运行二进制文件时更加简单：
      ```
      $ vim .bash_profile
      ...
      $ hello
      ```
    * 若使用源代码控制系统，需要初始化仓库，添加文件并提交第一个更改（可选）：
      ```
      $ git init
      $ git add go.mod hello.go
      $ git commit -m"initial commit"
      ```
      ```go```命令通过请求相应的```HTTP URL```并读取嵌入在```HTML```响应中的元数据来定位包含给定模块路径的仓库（[```go help```](https://pkg.go.dev/cmd/go#hdr-Remote_import_paths)）。许多托管服务已经为包含```GO```代码的仓库提供元数据，故使模块可供其他使用的最简单方法通常是使其模块路径与仓库的```URL```匹配。
    * 从本身模块导入包
      * 编写```morestrings```包，并在```hello```程序中使用。首先为名为```$HOME/hello/morestrings```的包创建一个目录，然后在该目录中创建一个名为```reverse.go```的文件：
        ```
        package morestrings
        ...
        ```
        因```ReverseRunes```函数以大写字母开头，故它可以被导出，且可以导入至其他包中使用。
      * 构建```morestrings```包
        ```
      ...
      * 修改```$HOME/hello/hello.go```使用```morestrings```包：
        ```
        package main
      ...
      * 运行新版本程序
        ...
        ```
    * 从远程模块导入包
      * 导入路径可以描述如何使用```Git```或```Mercurial```等版本控制系统获取包源代码。```go```工具使用此属性自动从远程仓库获取包。如程序中使用```github.com/google/go-cmp/```
        ```
        package main
        ...
        ```
      * 下载远程调用模块并将其版本记录在```go.mod```文件中：
        ```
        $ go mod tidy
        ```
        ```go mod tidy```命令为导入的包添加缺失的模块并删除不再使用的模块。
      * 构建并安装程序
        ```
        $ go install example/user/hello
        ```
      ...
      * 删除所有下载的模块
        ...
        $ go clean -modcache
        ```
        模块依赖会自动下载到```GOPATH```环境变量指定的```pkg/mod```子目录中。指定版本的模块下载的内容在需要该版本的所有其他模块之间共享，故这些文件和目录标记为只读。
  * 测试
    * ```GO```有一个由```go test```命令和测试包组成的轻量级测试框架。可以通过创建一个名称以```_test.go```结尾的文件来编写测试用例，该文件包含名为```TestXXX```且具有签名func(t *testing.T)的函数，测试框架会运行每个这样的程序，若函数调用失败，如```t.Error```或```t.Fail```，则测试失败。
    * 创建测试文件```reverse_test.go```：
      ```
      package morestrings
