...
> 使用标记接口定义类型
  * 标记接口是不包含任何方法声明的接口，该接口标记实现类的属性，如接口```Serializable```标记实现类的可序列化属性：
    ```
    ...
    ```
  * 标记接口与标记注解的区别：
    * 标记接口定义了一个由标记类实例实现的类型，标记注解则没有定义类型。标记接口类型在编译时捕获错误，标记注解在运行时捕获错误。
    * 标记接口可以更精确定位目标。若使用目标```ElementType.TYPE```声明注解类型，则可被应用于任何接口或类。假设有一个标记仅适用于特定实现的接口，将其定义为标记接口，拓展它保证所有标记类型是唯一且适用的子类型。
    * 标记注解是注解工具的一部分，允许基于注解的框架中保持一致性。
  * 标记接口与标记注解的使用场景：
    * 如果应用于除接口或类以外的任何程序元素，或者标记符合到使用大量注解的框架中，则使用标记注解。
    * 如果只能使用接口或类来实现或扩展接口，则使用标记接口。