...
> 保持失败原子性
  * 失败的方法调用应该使对象保持在被调用之前的状态，这种方法属性称为失败原子性。实现失败原子性的方式：
    * 设计不可变的对象。当不可变对象被创建之后就处于一致不可变的状态。
    * 对于在可变对象上执行操作的方法，通常在执行操作之前检查参数的有效性，使得在对象的状态被修改之前先抛出适当的异常。
    * 调整计算处理过程的顺序，使得任何可能会失败的计算部分都在对象状态被修改之前。
    * 在对象的临时拷贝上执行操作，当操作完成后再将拷贝上的结果赋予对象的内容。
    * 编写恢复代码。由恢复代码拦截操作过程中发生的失败，以及将对象回滚到操作开始之前的状态。这种方式适用于基于磁盘的数据结构。
  * 不需失败原子性的场景：
    * 若两个线程试图在没有适当的同步机制的情况下，并发修改同一对象时，无需保持失败原子性。
    * 发生的错误通常是不可恢复的，当抛出```AssertionError```时，无需保持失败原子性。

> 不要忽略异常
  * 不能使用空的```catch```块用于忽略异常。
  * 如果选择忽略异常，```catch```块应该包含注释，说明忽略原因且异常变量名应该命名为```ignored```。