...
> 不要依赖线程调度器
  * 当许多线程可以运行时，线程调度器可以决定哪些线程运行以及运行多长时间，但这些决定通常由操作系统作出的，只是策略有所不同，良好的应用程序不应该依赖于此策略的细节，任何依赖线程调度器来保证正确性或性能的程序都可能时不可移植的。
  * 编写健壮、响应快、可移植程序的最佳方法是确保可运行线程的平均数量不显著大于处理器的数量。如对于```Executor```框架，应该适当调整线程池的大小并保持任务短小，否则分派的开销依然会损害性能。
  * 不依赖```Thread.yield```或线程优先级。线程优先级可以少量地用于提高已经工作的程序的服务质量，但绝不应用于修复几乎不能工作的程序。