...
* 故障转移日志分析，了解故障转移过程中各节点的日志行为，对于运维人员快速定位和解决线上故障至关重要。作者通过使用 ```kill -9``` 模拟主节点宕机，记录并分析了整个过程：

  * 主节点 (```Master```) 行为：由于是模拟宕机，主节点不会产生任何日志。
  * 从节点 (```Slave```) 行为：
    * 从节点首先感知到与主节点失联，并持续尝试重连。
    * 在故障转移开始后，被选为新主节点的从节点会收到哨兵的指令，执行 ```slaveof no one``` 晋升为主节点，并重写配置文件（```CONFIG REWRITE```）。
    * 其他从节点会收到指令，开始从新的主节点进行复制。
  * 哨兵节点 (```Sentine```l) 行为：
    * 主观下线 (```+sdown```)：每个哨兵在 ```down-after-milliseconds``` 时间后检测到主节点不可达，将其标记为主观下线。
    * 客观下线 (```+odown```)：当达到 ```quorum``` 票数时，哨兵集体认为主节点不可达，标记为客观下线。
    * 领导者选举 (```+elected-leader```)：哨兵之间通过 ```Raft``` 算法选举出一个领导者来负责故障转移。
    * 执行转移：领导者按步骤执行：```select-slave```（选出最佳从节点）-> ```send-slaveof-noone```（晋升）-> ```reconf-slaves```（通知其他从节点）-> ```switch-master```（发布切换消息）。
  * 故障节点恢复：原主节点恢复后，哨兵会发现它并命令其去复制新的主节点，将其降级为从节点（```+convert-to-slave```）。
  * 运维提示：建议使用 ```NTP``` 服务保持所有节点时钟一致，否则日志的时序性会混乱，影响排查。

* 节点运维，在日常运维中，节点的上线和下线是常规操作，需要区分临时下线和永久下线：
  * 节点下线：
    * 主节点下线：最合理的做法是利用 ```sentinel failover <master-name>``` 命令在哨兵节点上发起手动故障转移。这可以确保选出一个性能更好的从节点晋升，减少由于自动发现导致的业务不可用时间。
    * 从节点/哨兵下线：确定下线类型后直接关闭即可。需要注意，即使从节点下线，哨兵仍会定期监控它（为了在它重连时自动恢复），这在 ```debug``` 日志中会体现为连接拒绝，造成少量资源浪费。
  * 节点上线：
    * 添加从节点：配置 ```slaveof``` 指向主节点并启动，哨兵会自动发现并将其纳入监控。
    * 添加哨兵节点：配置 ```sentinel monitor``` 同一组主节点并启动，其他哨兵和数据节点会自动感知该新哨兵。
  * 运维提示：哨兵节点支持的命令非常有限（如 ```ping```, ```sentinel```, ```info``` 等），不支持 ```config``` 命令。建议所有哨兵节点的配置尽可能一致，以保证故障判断的准确性。

* 高可用读写分离，虽然从节点可以扩展读能力，但在标准的 ```Redis Sentinel``` 中，从节点并不是天然高可用的。如果一个从节点故障，哨兵只会标记其主观下线，而不会像对主节点那样执行自动故障转移。
  * 设计思路：为了实现从节点的高可用读写分离，客户端需要实时掌握所有从节点的状态。
  * 利用消息通知：客户端可以订阅哨兵节点发出的事件消息（```Pub/Sub```）来更新本地维护的从节点资源池：
    * ```+switch-master```：主节点切换，说明原从节点晋升，资源池需移除该节点。
    * ```+convert-to-slave```：原主节点降级为从节点，说明添加了新从节点。
    * ```+sdown```：检测到从节点主观下线，客户端应将其从读资源池中移除。
    * ```+reboot```：节点重启，如果是从节点角色，可以重新加入读资源池。
    * 总结：通过将所有可用从节点看作一个动态资源池，并依赖哨兵的通知机制实时更新，可以达到从节点的高可用性。