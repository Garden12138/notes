...
* 备份与恢复数据：
  ...
  * 备份，必要时可使用命令```SAVE```或```BGSAVE```（后台异步执行）进行当前实例数据库的备份，备份所创建的的```dump.rdb```文件保存在安装目录下。查看安装目录可用命令如下：
    ```bash
    CONFIG GET dir
    ```
...
* 性能测试；
  ...
  * 性能测试，可在客户端执行```redis-benchmark```命令进行```redis```的性能测试：

    ```bash
    redis-benchmark [option] [option value]
    # 测试主机为127.0.0.1，端口号为6379，执行的命令为set,lpush，请求数为10000的性能
    ...
    | 序号 | 选项 | 说明 | 默认值 |
    | :--- | :--- | :--- | :--- |
    | 1 | -h | 指定服务可访问的主机名 | 127.0.0.1 |
    | 2 | -p | 指定服务可访问的端口 | 6379 |
    | 3 | -s | 指定服务的```socket``` |  |
    | 4 | -c | 指定并发连接数 | 50 |
    | 5 | -n | 指定请求数 | 10000 |
    | 6 | -d | 以字节形式指定```SET/GET```值的数据大小 | 2 |
    | 7 | -k | 指定是否保持连接，1为是（```keep alive```），0为否（```reconnect```） | 1 |
    | 8 | -r | ```SET/GET/INCR```使用随机```key```，```SADD```使用随机值 |  |
    | 9 | -P | 通过管道传输```numreq```请求 | 1 |
    | 10 | -q | 强制退出，仅仅显示```query/sec```值 |  |
    | 11 | -csv | 以```csv```格式输出 |  |
    | 12 | -l（```L```的小写形式） | 永久循环执行测试 |  |
    | 13 | -t | 指定测试的多个具体命令，以逗号相隔 |  |
...
* 分区：
  ...
  * 分区技术，是将```redis```中数据进行拆分，然后将拆分后的数据分散至多个不同的```redis```实例的过程，每个实例仅存储数据集的某一部分（子集）。分区的优势有：

    * 提升服务器的性能（利用多台计算机的算力和网络带宽）。
    * 提高服务器的存储能力（利用多台计算机存储容量的提升）。

    但同时存在不足之处：

    * 涉及多个```key```操作时，通常不支持。如两个```set```在不同的实例上，不能对这两个```set```进行交集操作。
    * 不支持包含多个```key```的```redis```事务。
    * 处理数据非常复杂。如处理多个```.rdb```或```.aof```存储文件，需从多个实例中备份数据。
    * 添加、删除实例变复杂，无法针对实例的变化进行对应的分区调整。如哈希分区所使用的实例数取模。

  * 常用的分区方式：

    * 范围分区，将特定范围的```key```映射至指定实例上，命名格式如下：

      ```bash
      object_name:<id> # user:1、user:2用来表示不同id的用户
      ```

      范围分区的局限性在于```key```必须可用范围表示且需维护```key```与实例的映射表。

    * 哈希分区，将```key```进行哈希运算再根据实例数取模，命名格式如下：
